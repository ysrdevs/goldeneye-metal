...
spv::Id SpirvShaderTranslator::ProcessVectorAluOperation(
  ...
  switch (instr.vector_opcode) {
    case ucode::AluVectorOpcode::kImplicitPosition: {
      if (instr.vector_and_constant_result.storage_target ==
        ...
        InstructionOperand implicit_position_operand;
        implicit_position_operand.storage_source = InstructionStorageSource::kRegister;
        implicit_position_operand.storage_index = 0;
        implicit_position_operand.storage_addressing_mode =
            InstructionStorageAddressingMode::kAbsolute;
        implicit_position_operand.component_count = 4;
        implicit_position_operand.components[0] = SwizzleSource::kX;
        implicit_position_operand.components[1] = SwizzleSource::kY;
        implicit_position_operand.components[2] = SwizzleSource::kZ;
        implicit_position_operand.components[3] = SwizzleSource::kW;
        spv::Id implicit_position_storage = LoadOperandStorage(implicit_position_operand);
        return GetOperandComponents(implicit_position_storage, implicit_position_operand,
  ...
  EmitTranslationError("Unknown ALU vector operation ignored for Metal/SPIR-V bring-up", false);
  return used_result_component_count
             ? const_float_vectors_0_[used_result_component_count - 1]
             : spv::NoResult;
}
...
spv::Id SpirvShaderTranslator::ProcessScalarAluOperation(
  ...
  std::snprintf(message, sizeof(message),
                "Unknown ALU scalar operation %u ignored for Metal/SPIR-V bring-up",
                uint32_t(instr.scalar_opcode));
  EmitTranslationError(message, false);
  return const_float_0_;
}
