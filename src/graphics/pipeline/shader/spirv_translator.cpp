...
void SpirvShaderTranslator::StartTranslation() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (IsSpirvRectangleListVertexLoopEnabled()) {
      ...
      // The rectangle-list scratch array must NOT reuse the Block-decorated
      // gl_PerVertex type (type_output_per_vertex_). As a Function-storage local
      // array element, SPIRV-Cross cannot redeclare gl_PerVertex and emits an
      // undeclared "_RESERVED_IDENTIFIER_FIXUP_gl_PerVertex" identifier in the
      // generated MSL, which fails pipeline compilation on MoltenVK and on the
      // native Metal backend (both consume SPIR-V -> SPIRV-Cross -> MSL). Mirror
      // the gl_PerVertex layout with a plain, undecorated struct -- identical
      // member ordering, so output_per_vertex_member_*_ indices and every access
      // chain below are unchanged -- which gets a real MSL struct declaration.
      auto rectangle_shader_modification = GetSpirvShaderModification();
      uint32_t rectangle_clip_distance_count =
          rectangle_shader_modification.GetVertexClipDistanceCount();
      uint32_t rectangle_cull_distance_count =
          rectangle_shader_modification.GetVertexCullDistanceCount();
      std::vector<spv::Id> rectangle_per_vertex_members;
      rectangle_per_vertex_members.push_back(type_float4_);  // kOutputPerVertexMemberPosition
      ...
      spv::Id type_rectangle_per_vertex =
          builder_->makeStructType(rectangle_per_vertex_members, "xe_rectangle_per_vertex_scratch");
...
std::vector<uint8_t> SpirvShaderTranslator::CompleteTranslation() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (rectangle_vertex_loop) {
      ...
      // Store current iteration outputs into the scratch array. The scratch
      // struct mirrors gl_PerVertex's member layout but is a distinct, plain
      // (non-Block) type, so copy member-by-member -- a whole-struct store would
      // be a SPIR-V type mismatch.
      {
        auto store_rectangle_per_vertex_member = [this, rectangle_guest_vertex_index_int](
                                                     uint32_t member_index) {
          id_vector_temp_.clear();
          id_vector_temp_.push_back(builder_->makeIntConstant(int(member_index)));
          spv::Id source_value =
              builder_->createLoad(builder_->createAccessChain(spv::StorageClassOutput,
                                                               output_per_vertex_, id_vector_temp_),
                                   spv::NoPrecision);
          id_vector_temp_.clear();
          id_vector_temp_.push_back(rectangle_guest_vertex_index_int);
          id_vector_temp_.push_back(builder_->makeIntConstant(int(member_index)));
          builder_->createStore(source_value, builder_->createAccessChain(
                                                  spv::StorageClassFunction,
                                                  var_main_rectangle_per_vertex_, id_vector_temp_));
        };
        store_rectangle_per_vertex_member(kOutputPerVertexMemberPosition);
        if (output_per_vertex_member_clip_distance_ != UINT32_MAX) {
          store_rectangle_per_vertex_member(output_per_vertex_member_clip_distance_);
        }
        if (output_per_vertex_member_cull_distance_ != UINT32_MAX) {
          store_rectangle_per_vertex_member(output_per_vertex_member_cull_distance_);
        }
      }
      uint32_t interpolators_remaining = GetModificationInterpolatorMask();
      uint32_t interpolator_index;
...
void SpirvShaderTranslator::StartFragmentShaderInMain() {
  ...
  if (edram_fragment_shader_interlock_) {
    ...
    // Initialize framebuffer outputs before guest instructions. Doing this in
    // completion would overwrite any translated color/depth stores.
...
spv::Id SpirvShaderTranslator::GetStorageAddressingIndex(
  ...
  if (is_float_constant && !constant_register_map.float_dynamic_addressing) {
    uint32_t static_storage_index =
...
void SpirvShaderTranslator::StoreResult(const InstructionResult& result, spv::Id value,
  ...
  switch (result.storage_target) {
    ...
    case InstructionStorageTarget::kColor: {
      ...
      if (!current_shader().writes_color_target(result.storage_index)) {
        static std::atomic<uint32_t> ignored_color_store_logs{0};
        uint32_t ignored_color_store_index =
            ignored_color_store_logs.fetch_add(1, std::memory_order_relaxed) + 1;
        if (ignored_color_store_index <= 16 || (ignored_color_store_index & 0xFF) == 0) {
          std::fprintf(stderr,
                       "[spirv] ignored color store#%u target=%u mask=0x%x "
                       "declared_targets=0x%x\n",
                       ignored_color_store_index, result.storage_index, used_write_mask,
