...
void ShaderInterpreter::Execute() {
  ExecuteWithInstructionBudget(0);
}

bool ShaderInterpreter::ExecuteWithInstructionBudget(uint32_t instruction_budget) {
  ...
  auto consume_instructions = [&](uint32_t count) {
    if (!instruction_budget) {
      return true;
    }
    if (instructions_executed > instruction_budget - std::min(instruction_budget, count)) {
      return false;
    }
  ...
  for (uint32_t cf_index = 0; !exec_ended; cf_index = cf_index_next) {
    ...
    switch (cf_opcode) {
      ...
      case ucode::ControlFlowOpcode::kLoopStart: {
        ...
        uint32_t loop_stack_index = state_.loop_stack_depth++;
        auto loop_constant = register_file_.Get<xenos::LoopConstant>(
            XE_GPU_REG_SHADER_CONSTANT_LOOP_00 + cf_loop_start.loop_id());
        state_.loop_constants[loop_stack_index] = loop_constant;
        uint32_t& loop_iterator_ref = state_.loop_iterators[loop_stack_index];
...
void ShaderInterpreter::ExecuteAluInstruction(ucode::AluInstruction instr) {
  ...
  if (vector_result_write_mask || vector_opcode_info.changed_state) {
    ...
    switch (vector_opcode) {
      case ucode::AluVectorOpcode::kImplicitPosition: {
        if (instr.is_export() &&
            ucode::ExportRegister(instr.vector_dest()) == ucode::ExportRegister::kVSPosition) {
          const float* implicit_position = GetTempRegister(0, false);
          for (uint32_t i = 0; i < 4; ++i) {
            vector_result[i] = implicit_position[i];
          }
        } else {
          for (uint32_t i = 0; i < 4; ++i) {
...
void ShaderInterpreter::ExecuteTextureFetchInstruction(ucode::TextureFetchInstruction instr) {
  float result[4] = {};
  if (texture_fetch_callback_ && instr.opcode() == ucode::FetchOpcode::kTextureFetch) {
    const float* source = GetTempRegister(instr.src(), instr.is_src_relative());
    float coordinates[4] = {};
    uint32_t coordinate_count =
        std::min<uint32_t>(4, xenos::GetFetchOpDimensionComponentCount(instr.dimension()));
    uint32_t swizzle = instr.src_swizzle();
    for (uint32_t i = 0; i < coordinate_count; ++i) {
      coordinates[i] = source[(swizzle >> (2 * i)) & 0x3];
    }
    texture_fetch_callback_(texture_fetch_callback_user_data_, instr, coordinates,
                            coordinate_count, result);
  }
  ...
  if (needed_dwords) {
    ...
    for (uint32_t i = 0; i < 4; ++i) {
      ...
      if (dword_address_dwords >= fetch_constant.address &&
        ...
        const uint32_t* memory_dword =
            memory_.TranslatePhysical<const uint32_t*>(sizeof(uint32_t) * dword_address_dwords);
        dword_value = xenos::GpuSwap(*memory_dword, fetch_constant.endian);
      }
      data[i] = dword_value;
    }
