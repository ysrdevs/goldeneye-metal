...
#undef TYPE
  ...
  out->Append(opcode_name ? opcode_name : "unknownVfetch");
  out->Append(' ');
  ...
  if (attributes.data_format != xenos::VertexFormat::kUndefined) {
    ...
    if (data_format_name) {
      ...
      out->AppendFormat(", DataFormat=unknown({})", uint32_t(attributes.data_format));
    }
  }
...
void ParsedTextureFetchInstruction::Disassemble(string::StringBuffer* out) const {
  ...
  out->Append(opcode_name ? opcode_name : "unknownTfetch");
  out->Append(' ');
...
void ParsedAluInstruction::Disassemble(string::StringBuffer* out) const {
  ...
  if (!is_vector_op_default_nop) {
    ...
    out->Append(vector_opcode_name ? vector_opcode_name : "unknownVectorAlu");
  ...
  if (!is_scalar_op_default_nop) {
    ...
    out->Append(scalar_opcode_name ? scalar_opcode_name : "unknownScalarAlu");
