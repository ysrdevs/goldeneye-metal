...
void SpirvShaderTranslator::CompleteFragmentShaderInMain() {
  ...
  // Pixel shaders may write colors through registers allocated by `alloc colors`
  // instead of explicit oC# exports. Move those allocated export registers into
  // the real fragment outputs before alpha test and render-target handling.
  uint32_t allocated_color_targets =
      current_shader().allocated_color_targets() & ~current_shader().explicit_color_targets();
  if (allocated_color_targets && var_main_registers_ != spv::NoResult) {
    uint32_t color_target_index;
    while (rex::bit_scan_forward(allocated_color_targets, &color_target_index)) {
      allocated_color_targets &= ~(UINT32_C(1) << color_target_index);
      spv::Id color_variable = output_or_var_fragment_data_[color_target_index];
      uint32_t register_index =
          uint32_t(ucode::ExportRegister::kExportData0) + color_target_index;
      if (color_variable == spv::NoResult || register_index >= register_count()) {
        continue;
      }
      id_vector_temp_.clear();
      id_vector_temp_.push_back(builder_->makeIntConstant(int(register_index)));
      spv::Id color_value = builder_->createLoad(
          builder_->createAccessChain(spv::StorageClassFunction, var_main_registers_,
                                      id_vector_temp_),
          spv::NoPrecision);
      builder_->createStore(color_value, color_variable);
      if (edram_fragment_shader_interlock_) {
        ...
        builder_->createStore(
            builder_->createBinOp(
                ...
                builder_->makeUintConstant(UINT32_C(1) << color_target_index)),
