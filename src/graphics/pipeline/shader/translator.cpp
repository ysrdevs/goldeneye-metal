    ...
    for (uint32_t j = 0; j < 2; ++j) {
      ...
      switch (cf.opcode()) {
        ...
        case ControlFlowOpcode::kAlloc: {
          ...
          instr.Disassemble(&ucode_disasm_buffer);
          if (type() == xenos::ShaderType::kPixel &&
              instr.type == ucode::AllocType::kPsColors) {
            uint32_t color_target_count = std::min<uint32_t>(uint32_t(instr.count) + 1, 4);
            uint32_t color_target_mask = (UINT32_C(1) << color_target_count) - 1;
            writes_color_targets_ |= color_target_mask;
            allocated_color_targets_ |= color_target_mask;
            register_static_address_bound_ =
                std::max(register_static_address_bound_,
                         uint32_t(ExportRegister::kExportData0) + color_target_count);
          }
        } break;
...
void Shader::GatherVertexFetchInformation(const VertexFetchInstruction& op,
  ...
  for (auto& vertex_binding : vertex_bindings_) {
    if (vertex_binding.fetch_constant == op.fetch_constant_index()) {
      if (fetch_instr.attributes.stride && vertex_binding.stride_words &&
          vertex_binding.stride_words != fetch_instr.attributes.stride) {
        continue;
      }
      if (!vertex_binding.stride_words) {
        vertex_binding.stride_words = fetch_instr.attributes.stride;
      }
...
void Shader::GatherTextureFetchInformation(const TextureFetchInstruction& op,
  ...
  if (!binding.fetch_instr.operand_count) {
    return;
  }
...
void Shader::GatherAluResultInformation(const InstructionResult& result, uint32_t exec_cf_index) {
  ...
  switch (result.storage_target) {
    ...
    case InstructionStorageTarget::kColor:
      ...
      explicit_color_targets_ |= uint32_t(1) << result.storage_index;
      break;
...
void ParseTextureFetchInstruction(const TextureFetchInstruction& op,
  ...
  switch (op.opcode()) {
    ...
    default:
      std::fprintf(stderr,
                   "[shader] unsupported texture fetch opcode=%u src=%u dst=%u const=%u dim=%u "
                   "src_swiz=0x%02x dst_swiz=0x%03x pred=%u cond=%u fetch_valid=%u "
                   "unnorm=%u filters(mag/min/mip/aniso)=%u/%u/%u/%u\n",
                   uint32_t(op.opcode()), op.src(), op.dest(), op.fetch_constant_index(),
                   uint32_t(op.dimension()), op.src_swizzle(), op.dest_swizzle(),
                   op.is_predicated() ? 1u : 0u, op.predicate_condition() ? 1u : 0u,
                   op.fetch_valid_only() ? 1u : 0u,
                   op.unnormalized_coordinates() ? 1u : 0u, uint32_t(op.mag_filter()),
                   uint32_t(op.min_filter()), uint32_t(op.mip_filter()),
                   uint32_t(op.aniso_filter()));
      std::fflush(stderr);
      REXGPU_ERROR(
          "ShaderTranslator::ParseTextureFetchInstruction: Unsupported texture "
          "fetch opcode {} src={} dst={} const={} dim={} src_swiz=0x{:02x} dst_swiz=0x{:03x} "
          "pred={} cond={} fetch_valid={} denorm={} filters(mag/min/mip/aniso)={}/{}/{}/{}",
          uint32_t(op.opcode()), op.src(), op.dest(), op.fetch_constant_index(),
          uint32_t(op.dimension()), op.src_swizzle(), op.dest_swizzle(),
          op.is_predicated() ? 1 : 0, op.predicate_condition() ? 1 : 0,
          op.fetch_valid_only() ? 1 : 0, op.unnormalized_coordinates() ? 1 : 0,
          uint32_t(op.mag_filter()), uint32_t(op.min_filter()), uint32_t(op.mip_filter()),
          uint32_t(op.aniso_filter()));
      instr.opcode = op.opcode();
      instr.opcode_name = "unsupportedTfetch";
...
uint32_t ParsedTextureFetchInstruction::GetNonZeroResultComponents() const {
  ...
  switch (opcode) {
    ...
    default:
      REXGPU_ERROR(
          "ShaderTranslator::ParsedTextureFetchInstruction: Unsupported texture "
          "fetch opcode {} has no non-zero result components",
          uint32_t(opcode));
      components = 0b0000;
      break;
  }
