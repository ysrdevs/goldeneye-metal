...
bool RenderTargetCache::Update(bool is_rasterization_done,
  ...
  for (uint32_t i = 0; i < edram_bases_sorted_count; ++i) {
    ...
    // wrapping.
    // GoldenEye fix: when only a SINGLE render target is bound (e.g. a depth-only
    // pass with no viewport, whose estimated height is the full surface height),
    // do NOT let it wrap around the end of the EDRAM back onto its own base. Such
    // a wrap claims the tiles [0..base] that still hold the previously resolved
    // color scene, so the subsequent color resolve finds depth as the owner of
    // base 0 and dumps depth instead of the scene (black cutscenes/weapon-switch).
    // A lone render target physically cannot exceed the EDRAM, so cap at its end.
    uint32_t range_end_tiles =
        (i + 1 < edram_bases_sorted_count)
            ? edram_bases_sorted[i + 1].first
            : (edram_bases_sorted_count == 1 ? xenos::kEdramTileCount
