...
bool PixelNear(const uint8_t* bgra, const std::array<uint8_t, 4>& expected,
               uint8_t tolerance = 1) {
...
int RunPipelineProbeTest() {
  @autoreleasepool {
    ...
    id<MTLBuffer> position_buffer =
        [device newBufferWithBytes:kPositions.data()
    ...
    if (bgra.size() != size_t(kWidth) * kHeight * 4) {
      std::fprintf(stderr,
                   "[metal_pipeline_probe_test] FAIL: readback size %zu, expected %zu\n",
    ...
    const uint8_t* center =
        bgra.data() + (size_t(kHeight / 2) * kWidth + kWidth / 2) * 4;
    ...
    std::fprintf(stdout,
                 ...
                 "pixels through a one-layer texture2d_array (%zu clear)\n",
                 textured_pixels, clear_pixels);
    return 0;
  }
}
