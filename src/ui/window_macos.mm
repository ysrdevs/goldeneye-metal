...
class MacOSWindow final : public Window {
 public:
  ...
  enum class FullscreenTransitionSource {
    kNone,
    kRuntime,
    kSystem,
  };
  ...
  void HandleFullscreenWillChange(bool entering) {
    // A SetFullscreen request marks its transition before calling AppKit. If
    // this notification doesn't match such a request, fullscreen was changed
    // by macOS (the green title-bar button or the system shortcut), so mirror
    // the new desired state back into the common Window object immediately.
    if (fullscreen_transition_source_ != FullscreenTransitionSource::kRuntime ||
        fullscreen_transition_target_ != entering) {
      fullscreen_transition_source_ = FullscreenTransitionSource::kSystem;
      fullscreen_transition_target_ = entering;
      OnDesiredFullscreenUpdate(entering);
    }
  }

  void HandleFullscreenDidChange(bool entered) {
    native_fullscreen_ = entered;
    fullscreen_transition_source_ = FullscreenTransitionSource::kNone;
    ...
    // AppKit can't reverse a native fullscreen animation in flight. If the
    // desired state changed during it, begin the requested reverse transition
    // now that the previous one is complete.
    ApplyFullscreenFromUIThread();
  }

  void HandleFullscreenEntryFailure() {
    if (!window_) {
      return;
    }
    native_fullscreen_ = ([window_ styleMask] & NSWindowStyleMaskFullScreen) != 0;
    fullscreen_transition_source_ = FullscreenTransitionSource::kNone;
    // Avoid an automatic retry loop when AppKit rejects the transition. The
    // next explicit SetFullscreen request can try again.
    OnDesiredFullscreenUpdate(native_fullscreen_);
  }

  void HandlePendingPaint() {
    // windowDidUpdate: is called once by each outer iteration of the manual
    // AppKit loop. Clear the request before painting so a request generated by
    // an ImGui drawer during this paint remains set for the next iteration.
  ...
  void HandleMouse(NSEvent* event, MouseAction action) {
    ...
    switch (action) {
      ...
      case MouseAction::kMove: {
        ...
        // relative deltas. Keep NSEvent's relative Y sign: negating it reverses
        // physical up/down motion for both right-stick mapping and title-owned
        // menu/camera input.
        const int32_t relative_x =
            macos::AccumulateRelativeMouseDelta([event deltaX], mouse_delta_residual_x_);
        const int32_t relative_y =
            macos::AccumulateRelativeMouseDelta([event deltaY], mouse_delta_residual_y_);
 ...
 private:
  bool OpenImpl() override {
    ...
    // Install a developer-supplied layer before making the view layer-backed.
  ...
  void ApplyNewFullscreen() override {
    // NSWindow is main-thread-only, while SetFullscreen may originate from the
    // guest/menu thread.
    if (app_context().IsInUIThread()) {
      ApplyFullscreenFromUIThread();
      return;
    }
    app_context().CallInUIThreadSynchronous([this] { ApplyFullscreenFromUIThread(); });
  }
  ...
  void RequestPaintImpl() override {
    // A paint may be requested by the guest thread, and an ImGui drawer asks
    // for its next frame from inside the current paint. Calling OnPaint here
    // synchronously made that latter request hit Window's reentrancy guard and
    // get dropped, leaving interactive UI frozen on its first frame. The
    // NSWindow update callback consumes this coalesced flag on the UI thread.
  ...
  void ApplyFullscreenFromUIThread() {
    assert_true(app_context().IsInUIThread());
    if (!window_ || fullscreen_transition_source_ != FullscreenTransitionSource::kNone ||
        native_fullscreen_ == IsFullscreen()) {
      return;
    }

    fullscreen_transition_source_ = FullscreenTransitionSource::kRuntime;
    fullscreen_transition_target_ = IsFullscreen();
  ...
  void UpdatePlatformMouseStateFromUIThread() {
    ...
    if (mouse_should_be_disassociated != mouse_disassociated_) {
      mouse_delta_residual_x_ = 0.0;
      mouse_delta_residual_y_ = 0.0;
  ...
  void UpdateDrawableSize() {
    ...
    NSView* content_view = [window_ contentView];
    const NSRect backing_bounds = [content_view convertRectToBacking:[content_view bounds]];
    CGFloat scale = [[window_ screen] backingScaleFactor];
    [metal_layer_ setContentsScale:scale];
    [metal_layer_ setDrawableSize:CGSizeMake(NSWidth(backing_bounds), NSHeight(backing_bounds))];
  }
  ...
  void UpdateActualSize(WindowDestructionReceiver& destruction_receiver) {
    ...
    NSView* content_view = [window_ contentView];
    const NSRect backing_bounds = [content_view convertRectToBacking:[content_view bounds]];
    const CGFloat backing_width = NSWidth(backing_bounds);
    const CGFloat backing_height = NSHeight(backing_bounds);
    OnActualSizeUpdate(
        backing_width > 0.0 ? static_cast<uint32_t>(std::lround(backing_width)) : 0,
        backing_height > 0.0 ? static_cast<uint32_t>(std::lround(backing_height)) : 0,
  ...
  FullscreenTransitionSource fullscreen_transition_source_ = FullscreenTransitionSource::kNone;
  bool fullscreen_transition_target_ = false;
  bool native_fullscreen_ = false;
  bool mouse_disassociated_ = false;
  bool owns_cursor_hide_ = false;
  double mouse_delta_residual_x_ = 0.0;
  double mouse_delta_residual_y_ = 0.0;
};
...
- (void)windowWillEnterFullScreen:(NSNotification*)notification {
  (void)notification;
  if (owner_) {
    owner_->HandleFullscreenWillChange(true);
  }
}
...
- (void)windowDidEnterFullScreen:(NSNotification*)notification {
  (void)notification;
  if (owner_) {
    owner_->HandleFullscreenDidChange(true);
  }
}
...
- (void)windowWillExitFullScreen:(NSNotification*)notification {
  (void)notification;
  if (owner_) {
    owner_->HandleFullscreenWillChange(false);
  }
}
...
- (void)windowDidExitFullScreen:(NSNotification*)notification {
  (void)notification;
  if (owner_) {
    owner_->HandleFullscreenDidChange(false);
  }
}
...
- (void)windowDidFailToEnterFullScreen:(NSWindow*)window {
  (void)window;
  if (owner_) {
    owner_->HandleFullscreenEntryFailure();
  }
}
