...
void Presenter::WaitForUITickFromUIThread() {
  ...
  while (true) {
    ...
    // Bound the wait: IDXGIOutput::WaitForVBlank() on the tick thread can block
    // forever if the output stops producing vblanks (display sleep, monitor
    // topology change, output lost) -- it then never advances the vblank counter
    // or signals this condition, which would freeze the window paint while the
    // guest keeps running. On timeout, fall back to painting so the screen stays
    // live (~30fps) until vblanks resume.
    if (dxgi_ui_tick_signal_condition_.wait_for(dxgi_ui_tick_lock,
