...
namespace shaders {
#include "../shaders/vulkan_spirv/immediate_ps.h"
#include "../shaders/vulkan_spirv/immediate_vs.h"
}  // namespace shaders
...
bool VulkanImmediateDrawer::EnsurePipelinesCreatedForCurrentRenderPass() {
  ...
  vertex_input_state.vertexAttributeDescriptionCount =
      uint32_t(rex::countof(vertex_input_attributes));
  ...
  if (pipeline_line_create_result != VK_SUCCESS) {
    REXLOG_ERROR("VulkanImmediateDrawer: Failed to create the line list pipeline");
...
uint32_t VulkanImmediateDrawer::AllocateTextureDescriptor() {
  ...
  if (texture_descriptor_pool_recycled_first_) {
    ...
    rex::bit_scan_forward(pool->recycled_bits, &local_index);
