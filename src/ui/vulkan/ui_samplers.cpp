...
std::unique_ptr<UISamplers> UISamplers::Create(const VulkanDevice* const vulkan_device) {
  ...
  for (int sampler_index = 0; sampler_index < kSamplerCount; ++sampler_index) {
    ...
    const VkResult sampler_create_result = dfn.vkCreateSampler(
        device, &sampler_create_info, nullptr, &ui_samplers->samplers_[sampler_index]);
    if (sampler_create_result != VK_SUCCESS) {
      REXLOG_ERROR(
          ...
          vk::to_string(vk::SamplerAddressMode(sampler_create_info.addressModeU)),
...
UISamplers::~UISamplers() {
  for (const VkSampler sampler : samplers_) {
    ...
    vulkan_device_->functions().vkDestroySampler(vulkan_device_->device(), sampler, nullptr);
  }
}

UISamplers::UISamplers(const VulkanDevice* vulkan_device) : vulkan_device_(vulkan_device) {
  assert_not_null(vulkan_device);
