...
// Default ON: clock scaling takes a global mutex (UpdateGuestClock) on every
// guest timebase read. The GoldenEye GPU-completion hook reads the timebase in
// a tight spin, so with scaling enabled that mutex is contended by dozens of
// threads to a standstill (a hard freeze). The no-scaling path computes the
// guest tick lock-free, which removes the freeze.
REXCVAR_DEFINE_BOOL(clock_no_scaling, true, "Clock",
...
// Native guest ticks.
// Atomic so the contended path can read it lock-free (see UpdateGuestClock).
...
// Update the guest timer for all threads. Lock-free.
//
// The guest clock is a PURE function of the host clock: guest = host * ratio.
// The previous design accumulated deltas under a global mutex. The guest's
// frame-limiter is a tight spin-wait that reads the timebase (mftb -> this)
// millions of times/sec; whenever a caller was preempted while holding that
// mutex, NO thread could advance the clock, so the spin never reached its target
// time and looped forever -> intermittent visual freeze (audio/input keep running
...
uint64_t UpdateGuestClock() {
  ...
  // overflow for millennia of uptime. Identical math to the clock_no_scaling path.
  auto ratio = guest_tick_ratio_;
  return host_tick_count * ratio.first / ratio.second;
}
