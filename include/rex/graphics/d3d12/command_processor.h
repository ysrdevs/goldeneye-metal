...
class D3D12CommandProcessor : public CommandProcessor {
 ...
 private:
  ...
  // ge colour-grade post-process (in-place on the guest output after gamma).
  // Layout must match the cbuffer in ge_grade.cs.hlsl (scalars only, 12 dwords).
  ...
  enum class GradeRootParameter : UINT {
    kConstants,
    kDestination,

    kCount,
  };
  Microsoft::WRL::ComPtr<ID3D12RootSignature> grade_root_signature_;
  Microsoft::WRL::ComPtr<ID3D12PipelineState> grade_pipeline_;
